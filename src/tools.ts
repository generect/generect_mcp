    ...
    flush() {
      ...
      for (const name of ordered) {
        ...
        const wrapped = async (args: any, extra: any) => {
          ...
          try {
            const handled = await entry.handler(args, extra);
            // Annotated here, once, rather than in each tool body: a marker that a
            // tool can forget to add is a marker that will be missing from exactly
            // the tool where it mattered. See src/testmode.ts.
